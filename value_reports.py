...
class ValueDistributionReport:
    def __init__(self, connection_string: str, use_plotly: bool = True, max_inline_bars: int = 20, use_altair: bool = True):
        ...
        self.view_suffix = "_view"
        self.use_altair = use_altair

    def _ensure_view(self, table_name: str) -> str:
        t = table_name.strip()
        return t if t.endswith(self.view_suffix) else f"{t}{self.view_suffix}"
    ...
    def _altair_categorical_chart(self, data: pd.DataFrame, col_name: str, null_text: str):
        ...
        show_param = alt.param(name='Show Values', value=False, bind=alt.binding_checkbox(name='Show values'))
        ...
        labels = (
            ...
            .encode(
                ...
                opacity=alt.when(show_param, alt.value(1.0)).otherwise(alt.value(0.0))
            )
        )

        title = alt.Chart(pd.DataFrame({'t': [f'{col_name} - Distribution  |  {null_text}']})).mark_text(
            align='left', dx=0, dy=-10, fontSize=12
        ).encode(text='t:N')

        chart = (bars + labels).properties(height=360).resolve_scale(y='independent')
        return alt.vconcat(title, chart)
    ...
    def _altair_hist_chart(self, clean_series: pd.Series, col_name: str, x_label: str, null_text: str):
        ...
        return alt.vconcat(title, hist.properties(height=360)).interactive()
    ...
    def create_overview_dashboard(self, table_name: str, max_cols: int = None) -> None:
        try:
            view_table = self._ensure_view(table_name)
            df = pd.read_sql(f"SELECT * FROM {view_table}", self.engine)
            if df.empty:
            ...
            for i, col in enumerate(columns_to_show):
                ...
                if col_type == 'dob':
                    ...
                    # Convert to numeric days to avoid Timedelta formatting issues
                    ...
                    if not display_data.empty:
                        bars = ax.bar(range(len(display_data)), display_data['Count'],
                                     color=self.current_palette[i % len(self.current_palette)])
                        ax.set_xticks(range(len(display_data)))
                        ax.set_xticklabels([str(x)[:8] + '...' if len(str(x)) > 8 else str(x)
                                            for x in display_data[col]], rotation=45, fontsize=8)
    ...
    def analyze_all_columns(self, table_name: str, top_n: int = 10, show_individual_plots: bool = True, max_columns: int = None, sample_size: int = None) -> Dict:
        try:
            ...
            self.create_overview_dashboard(view_table, max_cols=max_columns)
            return results
        except Exception as e:
    ...
    def create_complete_report(self, table_name: str, connection_string: str = None):
        if connection_string:
            self.engine = create_engine(connection_string)
        view_table = self._ensure_view(table_name)
        print(f"🚀 Complete Value Distribution Report for {view_table}")
        results = self.analyze_all_columns(table_name=view_table, top_n=15, show_individual_plots=True)
        return results

...
def download_report(job_id: str):
    ...
    try:
        ...
        # Define a loose threshold for fuzzy matching (e.g., 75%)
        FUZZY_THRESHOLD = 0.75

        # Select string columns to build a 'fingerprint' for each record, ignoring identifiers
        cols_to_use = [c for c in clusters_df.columns if c not in ['unique_id', 'cluster_id']]
        string_cols = clusters_df[cols_to_use].select_dtypes(include=['object', 'string']).columns

        if not list(string_cols):
             # Fallback if no string columns are available for matching
            clusters_df['partition_group'] = 'group_' + (pd.factorize(clusters_df['cluster_id'])[0] + 1).astype(str)
        else:
            # Create a 'fuzzy_key' fingerprint for each record
            clusters_df['fuzzy_key'] = clusters_df[string_cols].fillna('').astype(str).agg(' '.join, axis=1)

            # Get unique initial clusters and a representative key for each
            cluster_representatives = clusters_df.groupby('cluster_id')['fuzzy_key'].first().to_dict()
            unique_clusters = list(cluster_representatives.keys())
            
            # Use a Disjoint Set Union (DSU) data structure to efficiently merge clusters
            ...
            def union(c1_id, c2_id):
                root1, root2 = find(c1_id), find(c2_id)
                if root1 != root2:
                    parent[root2] = root1

            # Compare all pairs of cluster representatives and merge them if they are similar enough
            for i in range(len(unique_clusters)):
                for j in range(i + 1, len(unique_clusters)):
                    c1, c2 = unique_clusters[i], unique_clusters[j]
                    key1, key2 = cluster_representatives[c1], cluster_representatives[c2]
                    
                    # Use SequenceMatcher for dependency-free fuzzy string comparison
                    if SequenceMatcher(None, key1, key2).ratio() >= FUZZY_THRESHOLD:
                        union(c1, c2)

            # Map each original cluster_id to its new merged partition root
            cluster_to_partition_map = {c: find(c) for c in unique_clusters}
            clusters_df['partition_root'] = clusters_df['cluster_id'].map(cluster_to_partition_map)
            
            # Create the final user-friendly group names
            clusters_df['partition_group'] = 'group_' + (pd.factorize(clusters_df['partition_root'])[0] + 1).astype(str)
            clusters_df = clusters_df.drop(columns=['fuzzy_key', 'partition_root'])
        ...
        clusters_df = clusters_df.sort_values(by='partition_group')

...
// ================== MODIFIED FUNCTION FOR DIRTY RECORDS ==================
checkDuplicateBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // NEW: Handle the three possible outcomes from the backend, now with group info
    if (json.result === 'duplicate'){
      let detail = `Result: Duplicate (Group: ${json.partition_group}, Cluster: ${json.cluster_id})`;
      ...
      let detail = `Result: Potential Duplicate (Group: ${json.partition_group}, Score: ${probPercent}%) Cluster: ${json.cluster_id}`;
...
downloadLink.addEventListener('click', () => {
  ...
  try {
    ...
    // Immediately reset UI; the browser handles the download
...
resetBtn.addEventListener('click', async () => {
  ...
  // Representative UI removed; nothing to clear here
